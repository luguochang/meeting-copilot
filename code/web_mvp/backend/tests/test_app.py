...
def test_desktop_asr_worker_handoff_dry_run_readiness_reports_noop_boundary():
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["task_id"] == "task-096"
    assert payload["next_task_id"] == "task-106"
    ...
    assert payload["task_096_default_dry_run_status"] == "preview_ready_no_web_mutation"
...
def test_desktop_mic_adapter_contract_readiness_reports_contract_without_audio_access():
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["task_id"] == "task-106"
    assert payload["source_task_id"] == "task-105"
...
def test_desktop_real_mic_shadow_test_readiness_reports_static_gate_without_audio_access():
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["task_id"] == "task-115"
    ...
    assert "asr_quality_decision_requires_funasr_model_dir_or_task019_approval" in payload[
        "blockers"
    ]
...
def test_desktop_tauri_noop_run_result_validation_accepts_collector_result_without_running_tauri():
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["task_id"] == "task-113"
...
def test_desktop_realistic_meeting_simulation_pack_creates_richer_live_asr_session():
    ...
    assert payload["scenario_id"] == "task_126_release_incident_review"
...
def test_desktop_realistic_meeting_simulation_pack_long_shadow_profile_creates_report_preview():
    ...
    assert payload["scenario_id"] == "task_127_long_architecture_release_review"
...
def test_workbench_static_assets_are_served():
    ...
    assert "task_117_validation_status" in script.text
...
def test_web_mvp_readme_documents_scripted_browser_e2e_gate():
    ...
    task_076_plan = (
        REPO_ROOT
        / "docs"
        / "task-076-live-asr-card-lifecycle-append-result-audit-event-persistence-preflight-plan.md"
    ).read_text(encoding="utf-8")
    task_077_plan = (
        REPO_ROOT
        / "docs"
        / "task-077-live-asr-card-lifecycle-readiness-summary-plan.md"
    ).read_text(encoding="utf-8")
    task_078_plan = (
        REPO_ROOT
        / "docs"
        / "task-078-live-asr-card-lifecycle-readiness-ui-plan.md"
    ).read_text(encoding="utf-8")
    task_079_plan = (
        REPO_ROOT
        / "docs"
        / "task-079-desktop-shell-readiness-boundary-plan.md"
    ).read_text(encoding="utf-8")
    task_080_plan = (
        REPO_ROOT
        / "docs"
        / "task-080-desktop-runtime-boundary-plan.md"
    ).read_text(encoding="utf-8")
    task_080_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-080-desktop-runtime-boundary.md"
    ).read_text(encoding="utf-8")
    task_081_plan = (
        REPO_ROOT
        / "docs"
        / "task-081-desktop-native-bridge-contract-plan.md"
    ).read_text(encoding="utf-8")
    task_081_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-081-desktop-native-bridge-contract.md"
    ).read_text(encoding="utf-8")
    task_082_plan = (
        REPO_ROOT / "docs" / "task-082-tauri-shell-scaffold-spike-plan.md"
    ).read_text(encoding="utf-8")
    task_082_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-082-tauri-shell-scaffold-spike.md"
    ).read_text(encoding="utf-8")
    task_083_plan = (
        REPO_ROOT / "docs" / "task-083-desktop-build-readiness-policy-plan.md"
    ).read_text(encoding="utf-8")
    task_083_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-083-desktop-build-readiness-policy.md"
    ).read_text(encoding="utf-8")
    task_084_plan = (
        REPO_ROOT / "docs" / "task-084-desktop-cargo-check-artifact-policy-plan.md"
    ).read_text(encoding="utf-8")
    task_084_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-084-desktop-cargo-check-artifact-policy.md"
    ).read_text(encoding="utf-8")
    task_085_plan = (
        REPO_ROOT / "docs" / "task-085-desktop-rust-toolchain-readiness-plan.md"
    ).read_text(encoding="utf-8")
    task_085_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-085-desktop-rust-toolchain-readiness.md"
    ).read_text(encoding="utf-8")
    task_086_plan = (
        REPO_ROOT
        / "docs"
        / "task-086-desktop-rust-toolchain-installation-decision-plan.md"
    ).read_text(encoding="utf-8")
    task_086_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-086-desktop-rust-toolchain-installation-decision.md"
    ).read_text(encoding="utf-8")
    task_087_plan = (
        REPO_ROOT
        / "docs"
        / "task-087-desktop-rust-toolchain-install-approval-packet-plan.md"
    ).read_text(encoding="utf-8")
    task_087_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-087-desktop-rust-toolchain-install-approval-packet.md"
    ).read_text(encoding="utf-8")
    task_088_plan = (
        REPO_ROOT
        / "docs"
        / "task-088-desktop-rust-post-install-probe-approval-plan.md"
    ).read_text(encoding="utf-8")
    task_088_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-088-desktop-rust-post-install-probe-approval.md"
    ).read_text(encoding="utf-8")
    task_089_plan = (
        REPO_ROOT
        / "docs"
        / "task-089-desktop-rust-post-install-probe-result-intake-plan.md"
    ).read_text(encoding="utf-8")
    task_089_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-089-desktop-rust-post-install-probe-result-intake.md"
    ).read_text(encoding="utf-8")
    task_090_plan = (
        REPO_ROOT
        / "docs"
        / "task-090-desktop-first-cargo-check-execution-boundary-plan.md"
    ).read_text(encoding="utf-8")
    task_090_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-090-desktop-first-cargo-check-execution-boundary.md"
    ).read_text(encoding="utf-8")
    task_091_plan = (
        REPO_ROOT
        / "docs"
        / "task-091-tauri-noop-shell-local-run-smoke-plan.md"
    ).read_text(encoding="utf-8")
    task_091_implementation_plan = (
        REPO_ROOT
        / "docs"
        / "superpowers"
        / "plans"
        / "2026-07-02-task-091-tauri-noop-shell-local-run-smoke.md"
    ).read_text(encoding="utf-8")
    ...
    for document in [
        ...
        assert "task-076" in document
        assert (
            "llm-card-lifecycle-append-result-audit-event-persistence-preflights"
            in document
        )
    assert "transaction_run_id" in task_076_plan
    assert "append_run_id" in task_076_plan
    assert "repository_result_id" in task_076_plan
    assert "audit_repository_transaction_status" in task_076_plan
    assert "safe_to_mutate_events" in task_076_plan
    assert "safe_to_append_events" in task_076_plan
    for document in [
        task_077_plan,
        requirements_traceability,
        acceptance,
        ...
        assert "task-077" in document
        assert "llm-card-lifecycle-readiness-summaries" in document
    assert "card_lifecycle_summary_phase_count" in task_077_plan
    assert "source_preflight_status" in task_077_plan
    assert "card_lifecycle_safe_to_execute_llm" in task_077_plan
    assert "must not copy unscoped upstream `safe_to_*` fields" in task_077_plan
    for document in [
        task_078_plan,
        requirements_traceability,
        acceptance,
        ...
        assert "task-078" in document
        assert "llm-card-lifecycle-readiness-summaries" in document
    assert "card-lifecycle-readiness-panel" in task_078_plan
    assert "local contract probe" in task_078_plan
    assert "No real LLM call" in task_078_plan
    for document in [
        task_079_plan,
        requirements_traceability,
        acceptance,
        ...
        assert "task-079" in document
        assert "/desktop/shell-readiness" in document
    assert "desktop-readiness-panel" in task_079_plan
    assert "No microphone capture" in task_079_plan
    assert "desktop_safe_to_capture_audio" in task_079_plan
    for document in [
        task_080_plan,
        task_080_implementation_plan,
        requirements_traceability,
        ...
        assert "task-080" in document
        assert "/desktop/runtime-boundary" in document
    assert "tauri_first_electron_fallback" in task_080_plan
    assert "sidecar_worker_planned" in task_080_plan
    assert "desktop_runtime_safe_to_create_shell" in task_080_plan
    assert "Implementation status: completed" in task_080_implementation_plan
    ...
    assert "task-080 extends this passive startup read set" in decision_log
    for document in [
        task_081_plan,
        task_081_implementation_plan,
        requirements_traceability,
        ...
        assert "task-081" in document
        assert "/desktop/native-bridge-contract" in document
    assert "runtime.get_status" in task_081_plan
    assert "audio.capture_start" in task_081_plan
    assert "asr_worker.start" in task_081_plan
    assert "desktop_bridge_safe_to_create_native_bridge" in task_081_plan
    assert "create_tauri_shell_scaffold_against_bridge_contract" in task_081_plan
    assert "create_tauri_shell_spike" not in task_081_plan
    for document in [
        root_readme,
        readme,
        task_082_plan,
    ...
    assert "devUrl" in task_082_plan
    assert "frontendDist" in task_082_plan
    assert "http://127.0.0.1:8765/" in task_082_plan
    assert "Cargo.lock" in task_082_plan
    assert "package.json" in task_082_plan
    assert "Implementation status: completed" in task_082_implementation_plan
    ...
    for document in [
        ...
        assert "safe_to_run_cargo_check_now=false" in document
    assert "blocked_until_explicit_approval_and_toolchain" in task_084_plan
    assert "blocked_until_cargo_lock_and_cache_exist" in task_084_plan
    assert "Implementation Status" in task_084_implementation_plan
    for document in [
        root_readme,
        readme,
        task_085_plan,
    ...
    assert "xcode-select -p" in task_085_plan
    assert "presence_only" in task_085_plan
    for document in [
        root_readme,
        readme,
        task_086_plan,
        ...
        assert "safe_to_install_toolchain_now=false" in document
    assert "official_rustup" in task_086_plan
    assert "approved_network_download_policy_for_rustup" in task_086_plan
    for document in [
        root_readme,
        readme,
        task_087_plan,
    ...
    assert "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh" in task_087_plan
    assert "rustup-init.exe" in task_087_plan
    assert "approved_manual_user_run_only_boundary" in task_087_plan
    assert "manual_rustup_self_uninstall_only" in task_087_plan
    for document in [
        root_readme,
        readme,
        desktop_readme,
        ...
        assert "safe_to_run_cargo_check_now=false" in document
    assert "rustc --version" in task_088_plan
    assert "cargo --version" in task_088_plan
    assert "rustup --version" in task_088_plan
    assert "xcode-select -p" in task_088_plan
    assert "presence_only_no_path" in task_088_plan
    for document in [
        root_readme,
        readme,
        desktop_readme,
        ...
        assert "safe_to_run_cargo_check_now=false" in document
        assert "blocked_until_task_084_and_user_approval" in document
    for document in [
        root_readme,
        readme,
        desktop_readme,
    ...
    assert "task-057" in readme
    assert "task-058" in readme
    assert "task-059" in readme
    assert "task-060" in readme
    assert "task-061" in readme
    assert "task-063" in readme
    assert "task-064" in readme
    assert "task-065" in readme
    assert "task-066" in readme
