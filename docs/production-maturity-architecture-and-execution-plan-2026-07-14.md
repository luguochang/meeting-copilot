...
- [x] 安装包内链路不依赖手工 uvicorn、仓库路径或用户 venv。
- [ ] 真实 mic 和 system audio 均有非空帧、可听录音和明确设备状态。
- [x] local ASR `provider_mode=real`、`is_mock=false`。
...
### 17.3 2026-07-17 Provider、Tauri IPC 与 packaged AI 主链收口

本轮不增加新 ASR provider，也不扩大评测集，只关闭桌面主流程的三个阻塞。

1. 桌面 AI 配置使用 Mac Keychain / Windows Credential Manager 保存 API Key；`provider-config.json` 只保存 base URL、model 和 provider label。配置通过本机认证 token 同步到 packaged backend 的进程内 runtime override，packaged 模式移除继承的 `LLM_GATEWAY_*`。远端 base URL 必须是 HTTPS；显式 loopback IP 才允许 HTTP。保存、更新和清除为串行事务，backend 同步失败时恢复此前 secret、metadata 和 runtime config。
2. Tauri 2 remote-origin ACL 已收口。`tauri-build::AppManifest` 为应用 command 生成 allow permissions；启动随机后端后，host 只注册 `main` 窗口、本次精确 `http://127.0.0.1:<port>/*`、`local(false)` 的 runtime capability，再执行 `window.navigate`。不允许全端口 localhost、非 loopback host 或隐式端口。
3. React 只在显式 smoke flag 下运行无采音 boot probe；真实打包 WebView 调用 `runtime_get_status -> provider_config_status -> mic_adapter_prepare -> runtime_write_frontend_probe`。证据 `artifacts/tmp/packaged_tauri_ipc_smoke/phase3-packaged-tauri-ipc-20260717-r1/evidence.json` 的 8 项检查全为 true，且 app/backend/端口全部回收。该证据不启动麦克风、不绕过授权、不调用远端服务。
4. 同一新 `.app` 使用受控中文技术 WAV、本地 FunASR 和本地 OpenAI-compatible fake provider 完成 ASR、流式建议、修正、录音、结束会议和会后任务。证据为 `artifacts/tmp/packaged_ai_mainline_smoke/phase3-packaged-ai-mainline-20260717-r2/evidence.json`：1 个 final、3 个录音 chunk、14.842 秒 WAV，suggestion draft/delta/commit 和 correction/minutes/approach 各一次 provider 请求，index/minutes/approach 全部 succeeded。

当前仍是 `M3 Mac Internal Alpha candidate / public release No-Go`。未完成项固定为：用户从打包页面显式点击后的 native mic + relay + UI 同场 E2E；system audio 正式接线；自然多人中文技术会议准确率与术语热词；packaged CSP；签名、公证、staple、separate clean Mac；模型与二进制再分发授权。不得把独立通过的 IPC prepare、native helper 和 backend AI smoke 拼接成一次 UI 全链路证据。
