...
Meeting Copilot 的文档按用途分层。当前需求、架构、决策和验收文档放在 `docs/` 根目录，历史阶段文档放入 `docs/archive/`，交接材料放入 `docs/handoff/`。
...
| `docs/` 根目录 | 当前 SDD/TDD 计划、需求、架构、决策、评测和发布报告 | 以文档中明确的 evidence scope 为准 |
| `docs/archive/` | 历史 readiness 索引和已被当前主线取代的阶段文档 | 仅用于追溯，不作为当前状态依据 |
...
根目录的 `DECISION.md` 是项目状态冻结和反循环规则的特殊决策文件，保留在仓库根目录以兼容历史入口；其他已结束的阶段性方案和报告放入 `docs/archive/`。
