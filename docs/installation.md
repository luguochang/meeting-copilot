...
```text
7d077e35f1e67da059a1291865d858f95eaa25c7257387b298b80187cc405463
```
...
4. 等待平台、空间、清单和哈希校验完成。
5. 导入完成后按提示重启应用；“实时转写”和“文件转写”均显示可用，且不再提示重启，即表示激活完成。

客户端只需要导入 `.mcpkg` 文件。如果分发方同时提供同名 `.sha256` 文件，它只用于在导入前手工核对下载是否完整，不需要在客户端中选择或上传。
