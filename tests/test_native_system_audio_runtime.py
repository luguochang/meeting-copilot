...
def _load_packaged_gate():
    spec = importlib.util.spec_from_file_location("system_audio_packaged_gate", PACKAGED_GATE)
...
def _load_bundle_tool():
    spec = importlib.util.spec_from_file_location("system_audio_bundle_tool", BUNDLE_TOOL)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def built_helper(tmp_path_factory):
    ...
    output = tmp_path_factory.mktemp("system-audio") / "meeting-copilot-native-system-audio"
...
def test_runtime_manifest_and_tauri_register_one_system_audio_source_runtime():
    ...
    adapter_source = (DESKTOP_ROOT / "src-tauri/src/desktop_audio_adapter_runtime.rs").read_text(
        encoding="utf-8"
    )
    ...
    assert "audio.system_audio_is_active()" in lib_source
    assert "audio.microphone_status()" in lib_source
    assert "self.native_system_audio" in adapter_source
    assert "self.windows.start(" in adapter_source
    assert "AudioFlow::RenderLoopback" in adapter_source
    assert "app.manage(audio_adapters);" in lib_source
