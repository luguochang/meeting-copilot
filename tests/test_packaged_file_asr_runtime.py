...
def test_verified_package_association_and_report_paths_are_app_relative(tmp_path):
    ...
    assert association["run_id"] == "packaged-file-asr"
...
def test_package_evidence_mismatches_fail_closed(tmp_path, mutation, message):
    ...
    if mutation == "schema":
        ...
        payload["app_path"] = "artifacts/tmp/packaged-file-asr/Other.app"
...
@pytest.mark.skipif(
    not os.environ.get("MEETING_COPILOT_PACKAGED_APP")
    or not os.environ.get("MEETING_COPILOT_PACKAGE_EVIDENCE"),
    reason=(
        "set MEETING_COPILOT_PACKAGED_APP and MEETING_COPILOT_PACKAGE_EVIDENCE "
        "to run the multi-gigabyte real packaged model smoke"
    ),
)
def test_real_packaged_wav_m4a_mp3_worker_and_persistence(tmp_path):
    smoke = _load_script()
    fixture_root = Path(os.environ["MEETING_COPILOT_PACKAGED_ASR_FIXTURE_ROOT"])
    evidence = smoke.run_smoke(
        app_path=Path(os.environ["MEETING_COPILOT_PACKAGED_APP"]),
        package_evidence_path=Path(os.environ["MEETING_COPILOT_PACKAGE_EVIDENCE"]),
