#!/usr/bin/env python3
"""Build a bounded public-audio sample extraction plan without downloading audio."""
...
def _is_under_allowed_input_file_root(relative_path: Path) -> bool:
    path_text = relative_path.as_posix()
    return any(path_text == root or path_text.startswith(f"{root}/") for root in ALLOWED_INPUT_FILE_ROOTS)


def validate_input_file_path_before_read(path: str | Path, *, label: str) -> list[str]:
    raw_path = Path(path)
    ...
    if raw_relative is None:
        return [f"{label} path is outside repository"]
    if not _is_under_allowed_input_file_root(raw_relative) or not _is_under_allowed_input_file_root(
        resolved_relative
    ):
        return [f"{label} path is not under approved public audio metadata roots"]
    return []


def resolve_input_file_path_after_validation(path: str | Path, *, label: str) -> tuple[Path | None, list[str]]:
    errors = validate_input_file_path_before_read(path, label=label)
    if errors:
        return None, errors
    raw_path = Path(path)
    absolute_path = raw_path if raw_path.is_absolute() else REPO_ROOT / raw_path
...
def _validate_planned_samples(
    *,
    ...
    for index, sample in enumerate(planned_samples):
        ...
        missing_fields = [field for field in PLANNED_SAMPLE_REQUIRED_FIELDS if field not in sample]
        for field in missing_fields:
            errors.append(f"planned_samples[{index}].{field} is required")

        sample_id = _validate_non_empty_string(
            sample=sample,
            key="sample_id",
            index=index,
            errors=errors,
        )
        planned_source_id = _validate_non_empty_string(
            sample=sample,
            key="source_id",
            index=index,
            errors=errors,
        )
        planned_source_url = _validate_non_empty_string(
            sample=sample,
            key="source_url",
            index=index,
            errors=errors,
        ...
        archive_name = _validate_non_empty_string(
            sample=sample,
            key="archive_name",
            index=index,
            errors=errors,
...
def build_public_sample_extraction_plan(
    *,
    ...
    selection_criteria: str = "first manually reviewed meeting clips that fit the byte and duration budget",
    ...
    if source is None:
        ...
        resolved_source_split = source_split or source["default_source_split"]
...
def main(argv: list[str] | None = None, *, out: TextIO = sys.stdout) -> int:
    ...
    print(file=out)
    return 0 if report["plan_status"] == "ready_for_manual_download_review" else 1
